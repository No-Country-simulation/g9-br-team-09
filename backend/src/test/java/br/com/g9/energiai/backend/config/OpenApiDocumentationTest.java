...
class OpenApiDocumentationTest {
    ...
    @Test
    @DisplayName("Deve gerar a documentacao OpenAPI com metadados, endpoint e schemas esperados")
    void shouldGenerateOpenApiDocumentation() throws Exception {
        mockMvc.perform(get("/api/v1/v3/api-docs").contextPath("/api/v1"))
            ...
            .andExpect(jsonPath("$.paths['/analise-energetica'].post.responses['200'].content['*/*'].schema['$ref']")
                .value("#/components/schemas/EnergyAnalysisResponse"))
            .andExpect(jsonPath("$.paths['/analise-energetica'].post.responses['400'].content['*/*'].schema['$ref']")
                .value("#/components/schemas/ApiErrorResponse"))
            .andExpect(jsonPath("$.paths['/analise-energetica'].post.responses['500'].content['*/*'].schema['$ref']")
                .value("#/components/schemas/ApiErrorResponse"))
    ...
    @Test
    @DisplayName("Deve disponibilizar o Swagger UI no caminho publico com context-path")
