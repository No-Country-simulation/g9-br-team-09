...
Este diretório contém um smoke test reproduzível para validar o backend Spring Boot implantado na OCI Compute pela API HTTP real. As probes permanecem públicas; os cenários de autenticação e análise usam um access token obtido por login com um usuário técnico dedicado.
...
- um usuário técnico dedicado, persistente e previamente cadastrado;
- credenciais desse usuário armazenadas nos secrets do environment GitHub `oci-production`;
- uma janela controlada, sem outras gravações desse usuário na API durante a execução.

O script pode ser iniciado de qualquer diretório, pois resolve os fixtures em relação ao próprio arquivo. Ele não lê arquivos `.env`, não cadastra usuários e não recebe senha ou token por argumento de linha de comando. O deploy fornece somente o caminho de um arquivo temporário restrito com as credenciais do usuário dedicado.

Na workflow, os secrets obrigatórios são `OCI_SMOKE_USER_EMAIL` e `OCI_SMOKE_USER_PASSWORD`. Eles são materializados pelo job `deploy` em um arquivo local temporário com `umask 077`, copiados para o workspace remoto `0700`, protegidos como `0600` e removidos em sucesso ou falha. Não há valores funcionais padrão para essas credenciais.
...
| `SMOKE_AUTH_FILE` | sim | — | Caminho do arquivo temporário `0600` com e-mail e senha NUL-delimitados, preparado pelo deploy. |
...
SMOKE_AUTH_FILE=<ARQUIVO_TEMPORARIO_RESTRITO> \
REQUEST_TIMEOUT=15 \
...
4. login público em `POST /auth/login`, esperando HTTP `200` e `access_token` não vazio;
5. verificação autenticada da identidade em `GET /auth/me`, usando somente os campos `id`, `email` e `role` do contrato da resposta autenticada;
6. captura autenticada de `total_analises` pela API de resumo;
7. criação autenticada de uma análise válida e validação dos tipos do contrato público;
8. localização autenticada do registro no histórico pelo `id` devolvido no `POST`, com paginação, `snake_case` e ordenação decrescente;
9. consulta autenticada do detalhe pelo mesmo `id`, comparando entradas e campos calculados;
10. confirmação autenticada de que o total aumentou exatamente em uma unidade;
11. rejeição autenticada do payload inválido com HTTP `400` e `VALIDATION_ERROR`;
12. confirmação autenticada de que o payload inválido não foi persistido;
13. consulta autenticada de ID inexistente com HTTP `404` e `NOT_FOUND_ERROR`;
14. pós-condição da classificação: nova readiness pública no modo fallback ou confirmação de `ML_MODEL` no modo complementar.

`/auth/login` e `/actuator/**` são chamados sem `Authorization`. `/auth/me` e todas as chamadas de `/analise-energetica/**` recebem `Authorization: Bearer` com o token devolvido pelo login. O smoke não usa `/auth/register`, `/auth/refresh` ou `/auth/logout`; em particular, não lê nem depende do refresh token HttpOnly.

Todas as respostas são verificadas como JSON e inspecionadas contra indicadores óbvios de vazamento interno. O registro válido criado permanece persistido para o usuário técnico, pois a API não oferece uma operação de exclusão.
...
As mensagens usam somente os prefixos `[INFO]`, `[PASS]` e `[FAIL]`. Em falhas HTTP ou de contrato, o corpo completo não é exibido: o script mostra apenas metadados limitados e redige o restante. Erros de transporte também não reproduzem a saída detalhada do `curl`, para evitar revelar o destino. Uma falha de login não imprime o request, a resposta integral, a senha, cookies ou o access token.
...
- o arquivo de credenciais, seu conteúdo ou headers de autenticação;
...
O corpo do login e a resposta com o access token ficam apenas em um diretório temporário privado, criado com `mktemp -d` e modo `0700`; os arquivos são removidos automaticamente ao final, inclusive em falha. O access token é extraído para uma variável privada, nunca é decodificado ou gerado localmente e o header Bearer é entregue ao `curl` pela entrada padrão de configuração, não por argumento. O script não usa `JWT_SECRET`, não habilita `set -x` e não usa modo verboso do `curl`. O smoke não imprime o e-mail do usuário dedicado. A validação de conteúdo sensível nas respostas do smoke cobre `access_token`, `refresh_token`, cookies, `Authorization` e `Bearer`; ela não rejeita qualquer e-mail, pois `/auth/me` contém legitimamente o e-mail do usuário dedicado. A sanitização dos diagnósticos do deploy, adicionalmente, redige linhas que contenham e-mail.
...
- **Credenciais do smoke:** confirme que o usuário dedicado já existe, que os dois secrets do environment estão preenchidos e que o arquivo remoto temporário permanece regular, sem symlink e com modo `0600`.
- **Login HTTP 401:** valide a credencial do usuário técnico pelo procedimento operacional aprovado. Não registre a senha, cookies nem a resposta de login em evidências.
...
- A comparação e o histórico são do usuário técnico autenticado; a análise válida criada para ele não é removida ao final.
...
bash -n infra/tests/smoke/tests/test-backend-oci-smoke.sh
bash infra/tests/smoke/tests/test-backend-oci-smoke.sh
...
shellcheck \
  infra/tests/smoke/backend-oci-smoke.sh \
  infra/tests/smoke/tests/test-backend-oci-smoke.sh
```
